...
class customCN(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            "E. Koutsimari's CN Calculator: Calculates the Curve Number of a watershed based on Corine and SCS data"
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterVectorDestination(
                self.Corine, self.tr("Watershed with Corine Classes")
            )
        )
        self.addParameter(
            QgsProcessingParameterVectorDestination(
                self.Soil, self.tr("Watershed with SCS Classes")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            QgsMessageLog.logMessage(
                "Could not locate the Soil/Landuse data! Please make sure you have downloaded the datasets (https://drive.google.com/drive/folders/1uwq5Fixi8lvyIi0cdFhbjfmaBQsONNnK) and properly set up your settings, via the gear button on the plugin toolbar!",
                level=Qgis.Critical,
            )

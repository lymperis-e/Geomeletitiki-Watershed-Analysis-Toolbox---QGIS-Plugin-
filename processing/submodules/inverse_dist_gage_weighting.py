...
class InverseDistGageWeighting(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        outFields.append(
            QgsField("Σταθμισμένη i (mm/hr)", QVariant.Double, len=10, prec=4)
        )
        ...
        for _, feature in enumerate(features):
            ...
            total_distance = QgsExpression(
                'sum(1/(("Distance"/1000)*("Distance"/1000)))'
            ).evaluate(context)
    ...
    def shortHelpString(self):
        return self.tr(
            "Using the meteo stations parameters, calculate precipitation intensity via Inverse Distance Gage Weighting"
        )
